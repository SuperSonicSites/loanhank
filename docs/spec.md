...
**Self-refresh (amended 2026-09-02, owner ruling: no manual step).** The worker reads the AgDirect card itself: daily on the cron, and on the first ledger decode after a card lapses, at most once an hour. A fetched card is written only if it parses into the exact structure the seed has, passes the seed guards (band labels against bounds, terms against months, rate sanity, no gaps between bands), and moves no cell more than 300 basis points from the card it replaces; its source page is archived to `loanhank-snapshots` before any row points at it. Anything else is refused, the page is archived for a person, and the outcome is reported. Hand entry through a migration remains the fallback for a refused card, never a shortcut around the guards. Rates are still never guessed: a cell that does not parse as a rate is a refusal, not a zero.
...
Cloudflare Worker (Hono)
 ...
 ├─ POST /extract     → up to 4 photos of ONE paper → one merged vision LLM read → fields. farmer CONFIRMS every field. photos never written down.
...
**Calibration note (2026-08-17, beside the gates on purpose).** The click → completed decode gate above was calibrated on the four-field typed path. The camera-first hero may legitimately land lower on that gate while producing richer, verdict-capable rows, because a confirmed photo ledger can earn a stamp and a four-field decode never can. Read the first camera-hero flight against that expectation, not against the typed-path numbers alone. Events distinguish hero-path, disclosure-path, and failure-recovery decodes, and record photo count per decode, so the split is readable when the question comes up.
...
- **Verification traffic against production is flagged synthetic immediately, in the session that creates it.** Not at the end of the week, not when somebody asks, and not after a question about whether it counts. `decodes.synthetic`, `emails.synthetic` and `events.synthetic` are columns, and every published statistic and `ops/funnel.sql` read `synthetic = 0`.
  ...
  **Some things cannot be flagged after the fact.** A row can be marked synthetic an hour later and the pile stays honest. An outbound event cannot be un-sent. So verification traffic carries `x-loanhank-synthetic: 1` on the request, and the ads measurement gate (§10) refuses to fire on it. Note which way this fails: the header is opt-in, so forgetting it sends a real event. It is the second line of defence and not the first. Verification runs against the dev dataset or with `META_DATASET_ID` unset, and the header covers only the narrow case of proving the live wire. A stranger who sends the header merely opts their own decode out of measurement, which costs nothing and is roughly what GPC already does.
...
- **Gated on the synthetic header.** A request carrying `x-loanhank-synthetic: 1` fires nothing (§7.2). An event cannot be un-sent, so the synthetic decision has to be made at request time rather than flagged afterward like a row.
- **One event, not three.** `Decode` only. The canon copy on `/privacy` and `/straight-answers` says Meta is told that a decode happened and nothing else, and that sentence is the specification. The email gate and the interest question are measured in the `events` table by `ops/funnel.sql`, which is what Decision 8 is for. At $500 the downstream events are too thin for Meta to learn from anyway, so they would be data leaving for no gain.
...
- **Nothing persisted, in any table.** The `fbclid` rides a hidden form field from landing to POST and dies there. It never reaches `decodes`, `emails` or `events`, including inside `meta_json`. Meta's error bodies echo the offending value back, so a failure recorded verbatim would persist a click tag through the honest-logging path; only the error type and code are stored. Enumerating tables is how this clause first went wrong, so the rule is the whole table set and not a list.
...
**American English everywhere a farmer can read it.** "zero percent", never "nought". "anonymized", "subsidized", "organized", "license", "center", "analyze", "gray", "while" rather than "whilst".

This is not a style preference and it is not pedantry. A Nebraska farmer reading "nought per cent" hears a foreigner writing about his money, and the trust that costs does not come back. It sits with the fonts and the palette: a tell that the thing was not made for him.

Enforced by a dictionary check in the voice sweep (`tests/pages.test.ts`), which fails the build. Word boundaries throughout, because "analyses" is the correct American plural and a check that flags it is a check somebody deletes.

### 12.2 Notes

Papers, not a blog. No feed, no comments, no newsletter, no archive by month, no author byline, and no date parading as freshness. A note earns its place by being what a farmer wanted to read before he signed, and it comes down when it stops being that.

Every note obeys every law already in force: the voice sweep including the locale check, the promise registry, canon figures from design.md §2¾, a source and a date on every external number, and **exactly one ask, which is always to run the numbers**. A note that wants a second ask is two notes.

The template is tested against a stand-in note, so the rules bind before the first real paper is written rather than after four of them exist.
...
- [ ] **Ads measurement stays off until the privacy surface is live.** Measurement requires all of these to be true: the Do Not Sell or Share link is live and wired to a mechanism that works, the `/privacy` ads paragraph and GPC line are shipped, the `/straight-answers` answer is shipped, and the narrowed footer line is site-wide. All four through the voice sweep. CAPI is itself the CCPA share that triggers them (§10, lawyer stone 2), so shipping the code first is fine and switching measurement on first is not.

  **The switch is `META_CAPI_TOKEN`, not the dataset id.** The id is public and versioned in `wrangler.jsonc` beside `POSTAL_ADDRESS`, for the reason that comment gives: a value that enforces a promise does not belong in a store nobody reads, where a deploy can drop it and the promise quietly stops being kept. The sender needs both values, so an unset token is the enforcement and it is the only thing anyone has to flip.

  **Verified live 2026-08-18.** `GET /version` reported build `372524ec…-dirty`, and `372524e` is the commit that shipped the Track C privacy surface. The precondition is met; the token is what remains. The `-dirty` suffix is its own finding: production was deployed from a working tree, so the running artifact matches no commit anyone can name. Deploys belong to CI.
...
- [x] **Photo retention is code, not a dashboard click.** `infra/r2-lifecycle-quotes.json`, applied by `pnpm infra:r2`, which reads the rule back. The happy path never writes a photo at all: the bytes go from the request to the reader and are gone, which is a stronger promise than any sweep. The lifecycle rule caps the worst case at a day for any future path that does write one; the 15-minute cron logs that there is nothing to reap, so the slot stays visibly alive. (An earlier wording here credited a reaper module that never had an implementation; the never-written-down path is the actual mechanism, amended 2026-09-01.)
...
- [ ] **Cost + abuse protection on `/extract`.** Max upload size (`security.ts` has bones — verify), Turnstile invisible mode on submit, hard monthly spend cap + alert on the LLM provider account. An abuse script hitting `/extract` is a bill, not an outage. Turnstile belongs on the photo path only: it needs JS, and the typed form must keep working without any (design.md §9). **The camera hero reads up to four photos per decode (owner-approved cost), which multiplies the worst-case per-decode model bill; the provider spend cap is the backstop, and its being set is an owner-side precondition of the camera hero going live.**
